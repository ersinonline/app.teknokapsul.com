    ...
    <div aria-label="Abonelik Tablosu">
      {editingSubscription && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 lg:p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <h3 className="text-base lg:text-lg font-medium mb-4 text-gray-900 dark:text-white">Aboneliği Düzenle</h3>
            <SubscriptionForm
              onSubmit={async (data) => {
                if (!user) return;
                try {
            ...
            <button
              ...
              className="mt-4 w-full rounded-lg bg-gray-200 dark:bg-gray-600 px-4 py-2 text-gray-800 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-500"
            >
              İptal
            </button>
          </div>
              ...
              return (
                <tr key={subscription.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">{subscription.price} TL/ay</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        ...
                        className={`${subscription.isActive ? 'text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300' : 'text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300'} disabled:opacity-50`}
      ...
      <div className="lg:hidden space-y-3 p-3">
        {sortedSubscriptions.map((subscription) => {
          ...
          return (
            <div key={subscription.id} className="bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 p-4">
              <div className="flex justify-between items-start mb-3">
                <div className="flex-1">
                  <h3 className="text-sm font-medium text-gray-900 dark:text-white">{subscription.name}</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{subscription.price} TL/ay</p>
                </div>
                <span
              ...
              <div className="flex justify-end gap-2">
                <button
                  onClick={() => setEditingSubscription(subscription)}
                  className="p-2 text-yellow-600 hover:text-yellow-900 dark:text-yellow-400 dark:hover:text-yellow-300 hover:bg-yellow-50 dark:hover:bg-yellow-900/20 rounded"
                  title="Düzenle"
                >
                  <Edit2 className="w-4 h-4" />
                </button>
                <button
                  onClick={() => handleToggleStatus(subscription.id, subscription.isActive)}
                  className={`p-2 ${subscription.isActive ? 'text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 hover:bg-green-50 dark:hover:bg-green-900/20' : 'text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20'} disabled:opacity-50 rounded`}
                  title={subscription.isActive ? 'Pasif Yap' : 'Aktif Yap'}
                  disabled={isToggling === subscription.id}
                >
                  <Power className="w-4 h-4" />
