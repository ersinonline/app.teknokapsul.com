...
exports.processBankStatement = void 0;
...
function parseTRCurrencyToNumber(input) {
    const s = input.replace(/\s/g, '').replace(/\./g, '').replace(/,/g, '.');
    const n = Number(s.replace(/[^0-9.-]/g, ''));
    return isNaN(n) ? 0 : n;
}
function parseDateTR(d) {
    // format: dd.mm.yyyy
    const [dd, mm, yyyy] = d.split('.').map((x) => parseInt(x, 10));
    const date = new Date(Date.UTC(yyyy, mm - 1, dd));
    return date.getTime();
}
function detectType(line, amountStr) {
    const l = line.toLowerCase();
    if (l.includes('borç') || l.includes('çıkış') || amountStr.trim().startsWith('-'))
        return 'debit';
    if (l.includes('alacak') || l.includes('giriş') || amountStr.trim().startsWith('+'))
        return 'credit';
...
exports.processBankStatement = functions.storage.object().onFinalize(async (object) => {
    try {
...
//# sourceMappingURL=statementProcessor.js.map