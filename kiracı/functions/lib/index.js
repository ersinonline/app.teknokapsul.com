...
const fetchHolidaySet = async (ownerUid) => {
...
// Notification helper
export const createNotification = async (uid, type, title, message, actionUrl, metadata) => {
    try {
...
const computeInvoiceAmounts = (rentBase, agentUid) => {
    if (agentUid) {
...
export const createIyzicoCheckout = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        ...
        if (!input?.ownerUid || !input?.contractId || !input?.invoiceId) {
            res.status(400).json({ error: 'ownerUid, contractId, invoiceId zorunludur.' });
            return;
        }
        ...
        if (!apiKey || !secretKey || !callbackUrl) {
            res.status(500).json({ error: 'iyzico env değişkenleri eksik.' });
            return;
        }
        ...
        if (!invoiceSnap.exists) {
            res.status(404).json({ error: 'Invoice bulunamadı.' });
            return;
        }
        let invoice = invoiceSnap.data();
        if (invoice.status === 'PAID') {
            res.status(400).json({ error: 'Bu invoice zaten ödendi.' });
            return;
        }
        ...
        if (invoice.status === 'REFUNDED') {
            ...
            invoice = (await invoiceRef.get()).data();
        }
        ...
        try {
            const iyziRes = await new Promise((resolve, reject) => {
                iyzipay.checkoutFormInitialize.create(requestPayload, (err, result) => {
                    if (err)
                        reject(err);
                    else
                        resolve(result);
            ...
            if (!iyziRes || iyziRes.status !== 'success') {
                res.status(500).json({ error: 'iyzico checkout oluşturulamadı.', detail: iyziRes });
                return;
            }
            await invoiceRef.set({
                iyzico: {
...
export const guestGetContract = onCall({ cors: true }, async (request) => {
    const input = request.data;
    if (!input?.contractId || !input?.token)
        throw new HttpsError('invalid-argument', 'contractId ve token zorunludur.');
    ...
    if (!match)
        throw new HttpsError('not-found', 'Sözleşme bulunamadı.');
    const parts = match.ref.path.split('/');
    const ownerUid = parts[1];
    const contract = match.data();
    return {
        ownerUid,
        contractId: match.id,
        contract: {
...
export const guestListInvoices = onCall({ cors: true }, async (request) => {
    const input = request.data;
    if (!input?.contractId || !input?.token)
        throw new HttpsError('invalid-argument', 'contractId ve token zorunludur.');
    const tokenHash = sha256Hex(input.token);
...
export const guestCreateIyzicoCheckout = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
        ...
        if (!input?.contractId || !input?.invoiceId || !input?.token) {
            res.status(400).json({ error: 'contractId, invoiceId ve token zorunludur.' });
            return;
        }
        const apiKey = getEnv('IYZICO_API_KEY');
        const secretKey = getEnv('IYZICO_SECRET_KEY');
...
export const createUpfrontCheckout = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
        ...
        const body = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
        const { ownerUid, contractId, offerId } = body;
        if (!ownerUid || !contractId || !offerId) {
            res.status(400).json({ error: 'ownerUid, contractId, offerId zorunludur.' });
            return;
        }
        const apiKey = getEnv('IYZICO_API_KEY');
        const secretKey = getEnv('IYZICO_SECRET_KEY');
        ...
        if (offer.status !== 'ACCEPTED') {
            res.status(400).json({ error: 'Teklif henüz onaylanmamış.' });
            return;
        }
        if (offer.paidAt) {
            res.status(400).json({ error: 'Bu teklif zaten ödendi.' });
            return;
        }
        ...
        const isTenant = callerEmail && contract.tenant?.email && String(contract.tenant.email).toLowerCase() === String(callerEmail).toLowerCase();
        if (!isTenant) {
            res.status(403).json({ error: 'Sadece kiracı ödeme yapabilir.' });
            return;
        }
...
export const createIndependentCheckout = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
        ...
        const body = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
        const { paymentId, rentAmount: rentStr, landlordName, description } = body;
        if (!paymentId || !rentStr) {
            res.status(400).json({ error: 'paymentId ve rentAmount zorunludur.' });
            return;
        }
        const apiKey = getEnv('IYZICO_API_KEY');
        const secretKey = getEnv('IYZICO_SECRET_KEY');
...
export const iyzicoWebhook = onRequest({ region: 'europe-west1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        ...
        try {
            ...
            if (apiKey && secretKey) {
                try {
                    ...
                    retrieveResult = await new Promise((resolve, reject) => {
                        iyzipay.checkoutForm.retrieve({ locale: 'tr', conversationId: String(Date.now()), token }, (err, result) => (err ? reject(err) : resolve(result)));
                    });
                    ...
                    if (ps === 'SUCCESS' || rs === 'success')
                        status = 'SUCCESS';
                    else if (ps === 'FAILURE' || rs === 'failure')
                        status = 'REJECTED';
                    else
                        status = 'PENDING';
                }
            ...
            if (standaloneSnap && !standaloneSnap.empty) {
                ...
                const spData = (await spRef.get()).data();
                if (mapped) {
                    await spRef.set({
                        status: mapped,
                ...
                if (mapped === 'PAID' && spData?.type === 'DEPOSIT') {
                    ...
                    try {
                        ...
                        console.error('deposit mark error:', e?.message);
                    }
                }
                const resultStatus = mapped === 'PAID' ? 'success' : mapped === 'FAILED' ? 'failed' : 'pending';
                res.redirect(302, `${siteUrl}/payment-result?status=${resultStatus}`);
...
export const generateInvoicesDaily = onSchedule({ schedule: 'every day 01:05', timeZone: 'Europe/Istanbul', region: 'europe-west1' }, async () => {
...
export const recalcLateFees = onSchedule({ schedule: 'every day 00:20', timeZone: 'Europe/Istanbul', region: 'europe-west1' }, async () => {
...
export const generateInvoicesForContract = onCall({ cors: true }, async (request) => {
    if (!request.auth)
...
const generateInvoicesForContractInternal = async (params) => {
    ...
    const isTenant = params.authEmail &&
    ...
    const startDate = contract.startDate?.toDate ? contract.startDate.toDate() : null;
    if (!startDate)
        throw new HttpsError('failed-precondition', 'Sözleşme başlangıç tarihi eksik.');
    const payDay = clampPayDay(Number(contract.payDay || 1));
    const base = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate());
    const startMonthDue = new Date(base.getFullYear(), base.getMonth(), payDay);
    const firstDue = base.getDate() > payDay ? new Date(base.getFullYear(), base.getMonth() + 1, payDay) : startMonthDue;
...
export const generateInvoicesForContractHttp = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
...
export const schedulePayoutsDaily = onSchedule({ schedule: 'every day 02:05', timeZone: 'Europe/Istanbul', region: 'europe-west1' }, async () => {
    const snap = await admin
        .firestore()
...
export const renewalAutomation = onSchedule({ schedule: 'every day 03:05', timeZone: 'Europe/Istanbul', region: 'europe-west1' }, async () => {
...
const iyzicoRequest = (method, params) => {
...
export const checkPaymentStatus = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
        ...
        try {
            await admin.auth().verifyIdToken(tokenStr);
        }
        catch {
            res.status(401).json({ error: 'invalid-token' });
            return;
        ...
        const baseUrl = getEnv('IYZICO_BASE_URL') || 'https://sandbox-api.iyzipay.com';
        if (!apiKey || !secretKey) {
            res.status(500).json({ error: 'iyzico env eksik.' });
            return;
        }
        try {
            const Iyzipay = (await import('iyzipay')).default;
            ...
            if (invoice.status === 'PAID' && (ps === 'FAILURE' || (result?.price === 0) || (result?.itemTransactions || []).some((item) => item.transactionStatus !== 2))) {
            ...
            if (mapped && mapped !== invoice.status) {
                ...
                if (mapped === 'PAID') {
                    const inv = (await invoiceRef.get()).data();
                    const net = typeof inv?.landlordNet === 'number' ? inv.landlordNet : null;
                    const paidAt = inv?.paidAt?.toDate ? inv.paidAt.toDate() : new Date();
                    const holidaySet = await fetchHolidaySet(ownerUid);
                    const plannedBase = new Date(paidAt.getTime());
...
// ─── Deposit Checkout ──────────────────────────────────────────────────
export const createDepositCheckout = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
        ...
        if (!ownerUid || !contractId || !amount) {
            res.status(400).json({ error: 'ownerUid, contractId, amount zorunludur.' });
            return;
        }
        const apiKey = getEnv('IYZICO_API_KEY');
        const secretKey = getEnv('IYZICO_SECRET_KEY');
        ...
        if (contract.depositPaid) {
            res.status(400).json({ error: 'Depozito zaten ödendi.' });
            return;
        }
        const callerEmail = decoded.email;
        const isLandlord = contract.landlordUid === decoded.uid;
...
export const adminRefundPayment = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
        ...
        const invoice = invoiceSnap.data();
        if (invoice.status === 'REFUNDED') {
            res.status(400).json({ error: 'Bu fatura zaten iade edilmiş.' });
            return;
        }
        if (invoice.status !== 'PAID') {
            res.status(400).json({ error: 'Sadece PAID durumundaki faturalar iade edilebilir.' });
            return;
        }
        const paymentId = invoice.iyzico?.paymentId;
        if (!paymentId) {
            ...
            const checkoutToken = invoice.iyzico?.checkoutToken;
            if (!checkoutToken) {
                res.status(400).json({ error: 'iyzico paymentId veya checkoutToken bulunamadı.' });
                return;
            }
            const apiKey = getEnv('IYZICO_API_KEY');
            const secretKey = getEnv('IYZICO_SECRET_KEY');
            ...
            try {
                ...
                if (!formResult?.paymentId) {
                    res.status(400).json({ error: 'iyzico paymentId alınamadı.' });
                    return;
                }
                // Save paymentId for future
                await invoiceRef.set({ iyzico: { paymentId: formResult.paymentId } }, { merge: true });
                ...
                res.status(500).json({ error: 'iyzico paymentId sorgulama hatası.', detail: e?.message });
                return;
            }
        }
        ...
        try {
            ...
            if (!paymentResult?.itemTransactions?.length) {
                res.status(400).json({ error: 'iyzico ödeme detayları alınamadı.', detail: paymentResult });
                return;
            }
            ...
            try {
                ...
                if (contract?.tenant?.email) {
                    ...
                    if (!tenantQuery.empty) {
                        ...
                        if (tenantUid) {
                            await createNotification(tenantUid, 'system', 'Ödeme İade Edildi', `${invoice.period || ''} dönemi ödemeniz iade edildi. Tekrar ödeme yapmanız gerekmektedir.`, '/invoices');
                        }
                    }
                }
            }
...
export const adminUpdateInvoiceStatus = onRequest({ region: 'us-central1' }, async (req, res) => {
    corsHandler(req, res, async () => {
        if (req.method !== 'POST') {
            res.status(405).send('Method Not Allowed');
            return;
        ...
        if (!ownerUid || !contractId || !invoiceId || !newStatus) {
            res.status(400).json({ error: 'ownerUid, contractId, invoiceId, newStatus zorunludur.' });
            return;
        }
        if (!validStatuses.includes(newStatus)) {
            res.status(400).json({ error: `Geçersiz durum. Geçerli: ${validStatuses.join(', ')}` });
            return;
        }
        const invoiceRef = admin.firestore().doc(`accounts/${ownerUid}/contracts/${contractId}/invoices/${invoiceId}`);
        const invoiceSnap = await invoiceRef.get();
