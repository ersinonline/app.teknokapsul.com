...
export const sendPaymentReminders = onSchedule(
    { schedule: 'every day 09:00', timeZone: 'Europe/Istanbul', region: 'europe-west1' },
    async () => {
        const now = new Date();
        ...
        for (const invDoc of snap.docs) {
            ...
            if (tenantEmail) {
                try {
                    ...
                    await createNotification(
                        ...
                        'payment_due',
                        '💳 Ödeme Hatırlatması',
                        `${inv.period || ''} dönemi kira ödemeniz ${daysUntilDue} gün içinde yapılmalıdır. Tutar: ${inv.tenantTotal || 0} ₺`,
                        `/invoices`,
                        { contractId, invoiceId, dueDate: dueDate.toISOString() }
                    );
            ...
            await createNotification(
                ...
                'payment_due',
                '📅 Ödeme Hatırlatması',
                `${contract.tenant?.name || 'Kiracı'} için ${inv.period || ''} dönemi ödeme ${daysUntilDue} gün içinde yapılmalıdır.`,
...
export const sendOverdueNotifications = onSchedule(
    { schedule: 'every day 10:00', timeZone: 'Europe/Istanbul', region: 'europe-west1' },
    async () => {
        const snap = await admin
            .firestore()
            .collectionGroup('invoices')
        ...
        for (const invDoc of snap.docs) {
            ...
            if (lastNotifSent) {
                ...
                if (daysSinceLastNotif < 1) continue; // Already sent today
            }
            ...
            if (tenantEmail) {
                try {
                    ...
                    await createNotification(
                        ...
                        'payment_due',
                        '⚠️ Gecikmiş Ödeme',
                        `${inv.period || ''} dönemi kira ödemeniz ${daysOverdue} gündür gecikmiştir. Lütfen en kısa sürede ödeme yapınız.`,
                        `/invoices`,
                        { contractId, invoiceId, daysOverdue }
                    );
                } catch (e) {
                    console.error(`Tenant not found for email ${tenantEmail}:`, e);
                }
            ...
            await createNotification(
                ...
                'payment_due',
                '⚠️ Gecikmiş Ödeme',
                `${contract.tenant?.name || 'Kiracı'} için ${inv.period || ''} dönemi ödeme ${daysOverdue} gündür gecikmiştir.`,
                `/contracts/${contractId}`,
                { contractId, invoiceId, daysOverdue }
            );
